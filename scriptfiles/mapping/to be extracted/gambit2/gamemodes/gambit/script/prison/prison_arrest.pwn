...
function givePrisonInv( playerid, string ) {
	clear_massive();
	new rows, fields;
		...
		case d_arrest: {
			...
			switch( listitem ) {
				...
				case 3: {
					...
					format:query( "%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d",
					...
					format( vb_string, sizeof vb_string, "INSERT INTO `gb_prison` ( `pName`, `pReason`, `pDate`, `pExitDate`,\
						...
						Player[ giveplayerid ][uArrestReason], 
						gettime(), 
						Player[ giveplayerid ][uArrestTime],
						query
					);
					...
					format( vb_string, sizeof vb_string, "%s, '%s', '%s', '1' )", 
						vb_string,
						query
					);
...
stock setPrisonSpawnLogin( playerid, &Float:x, &Float:y, &Float:z ) {
	...
	format( vb_string, sizeof vb_string, "INSERT INTO `gb_prison` ( `pName`, `pReason`, `pDate`, `pExitDate`,\
		...
		Player[ playerid ][uArrestReason], 
		gettime(), 
		Player[ playerid ][uArrestTime],
		query
	);
