...
hook OnGameModeInit() {
	// - Магазин мебели
    CreateDynamicPickup( 1239, 23, -487.4362,-262.6086, 2095.9097, -1 );
    CreateDynamic3DTextLabel( "[ Покупка мебели ]\n\n"cBLUE"ALT", 0xFFFFFFFF, -487.4362,-262.6086,2095.9097, 5.0, INVALID_PLAYER_ID,INVALID_VEHICLE_ID,1);
	return true;
}
...
stock OpenFurnitureList( playerid, page ) {
	...
	showPlayerDialog( playerid, d_house + 13, 2, "Управление мебелью", vb_string, "Далее", "Закрыть");
	return true;
}
...
hook OnDialogResponse( playerid, dialogid, response, listitem, inputtext[] ) {
	...
	switch( dialogid ) {
		case d_house + 13: {
		    ...
		    return showPlayerDialog(playerid, d_house + 14, 2, "Управление мебелью", gb_string, "Далее", "Закрыть");
		}
		case d_house + 14: {
...
hook OnPlayerKeyStateChange( playerid, newkeys, oldkeys ) {
	switch( newkeys ) {
		case KEY_WALK: {
			if( IsPlayerInRangeOfPoint( playerid, 1.0, -487.4362,-262.6086,2095.9097 ) ) {
				new slot;
				for(new i; i != cBizz; i++) {
