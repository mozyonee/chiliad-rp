////////////////////////////////////////////////////////////////////////////////
//	A Recommended Update to v0.3 is Available - http://pastebin.com/XiKm1X56  //
////////////////////////////////////////////////////////////////////////////////
...
new AttachedObjectBones[MAX_ATTACHED_OBJECT_BONE][16] = {
...
public OnFilterScriptInit()
{
	...
	print("  Current Version: " #AOE_VERSION);
...
{
	printf("  Attached Objects Count: %d", GetAttachedObjectsCount());
    for(new x = 0, j = GetMaxPlayers(); x < j; x++) if(IsPlayerConnected(x)) AOE_UnsetVars(x);
	...
	else
	{
		...
		else
		{
			...
			if(IsValidPlayerAttachedObject(playerid, slot) != 1) SendClientMessage(playerid, COLOR_RED, "Warning: This attached object has unknown data, please save it first to refresh the data!");
		 	...
		 	if(pao[playerid][slot][aoBoneID] == 2) SendClientMessage(playerid, COLOR_YELLOW, "** Hint: Type {FFFFFF}/HEADMOVE{FFFF00} to toggle player head movement");
   		}
	}
	return 1;
}
...
stock GetAttachedObjectsCount()
{
	new _AttachedObjectsCount;
 	for(new x = 0, j = GetMaxPlayers(); x < j; x++)
