...
stock moveHouseItemTD( playerid ) {
	...
	if(HouseInfo[h][hInvSlot][slot_use] == 0) {
		...
		format( gb_string, sizeof gb_string, "переложил%s что-то в доме", Player[playerid][uSex] == 1 ? (""):("а") );
        MeAction( playerid, gb_string, 1 );
		saveHouseItem( h );
	}
...
stock takeHouseItemTD( playerid ) {
	...
	if( HouseInfo[h][hInvSlot][slot_change_use] != 0) {
		...
		format( gb_string, sizeof gb_string, "вз€л%s что-то из дома", Player[playerid][uSex] == 1 ? (""):("а") );
        MeAction( playerid, gb_string, 1 );
		saveHouseItem( h );
	}	
	return true;
