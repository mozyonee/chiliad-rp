...
CMD:enterexit( playerid, params[] ) {
	...
	for( new e; e < sizeof EnterInfo; e++ ) {
		if( EnterInfo[e][eID] == params[0] ) {
			...
			SendMes( playerid, C_WHITE, ""gbAccess"Для добавления комментария к пикапу введите - "cBLUE"/enterdesc %d [ описание ]"cWHITE".",
				EnterInfo[e][eID]
			);
...
CMD:enterdesc( playerid, params[] )
{
	if( !GetAccessAdmin( playerid, 7 ) )
		return SendMes( playerid, C_WHITE, !NO_ACCESS );
		
	if( sscanf( params, "ds[128]", params[0], params[1] ) ) 
		return SendMes( playerid, -1, ""gbError"Введите: /enterdesc [ ID ] [ Описание ]" );
		
	for( new e; e < sizeof EnterInfo; e++ ) 
	{
		if( EnterInfo[e][eID] == params[0] ) 
		{
			format( query, sizeof query, "UPDATE `gb_enters` SET `eDescription` = '%s' WHERE `eID` = %d LIMIT 1", 
				params[1],
				EnterInfo[e][eID] 
			);
			...
			SendMes( playerid, -1, ""gbAccess"Вы успешно добавили комментарий [%s] ID:%d",
				EnterInfo[e][eName], EnterInfo[e][eID] 
			);
...
function loadEnters() {
	...
	for( new j; j < rows; j++ ) {
		...
		sscanf( query,"e<p<|>iffffffiis[32]iii>", EnterInfo[j] );
		...
		CreateDynamic3DTextLabel( "ALT", C_BLUE, EnterInfo[j][ePe][0], EnterInfo[j][ePe][1], EnterInfo[j][ePe][2], 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, EnterInfo[j][eInt][1] );
		        
	}
	printf("[SYSTEM] Входы в интерьеры загружены [%d][time %d ms]", rows, GetTickCount() - time );
	return true;
}
		...
		case KEY_WALK: {
			for( new i; i < sizeof EnterInfo; i++ ) {
				if( IsPlayerInRangeOfPoint( playerid, 2.0, EnterInfo[i][eP][0], EnterInfo[i][eP][1], EnterInfo[i][eP][2] ) ) {
					if( EnterInfo[i][eMember] != 0 && EnterInfo[i][eMember] != Player[playerid][uMember] )
						continue;
