	...
	if( PRESSED( KEY_CROUCH ) ) 
	{
		if(IsPlayerInAnyVehicle(playerid)) 
		{
			if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER) 
			{
				...
				{
					if( !IsOwnerCar( GetPlayerVehicleID( playerid ), playerid ) )
						return 1;
						
					showPlayerDialog( playerid, d_repair, 2, " ", "Перекрасить авто\nКапитальный ремонт двигателя\nУстановка дисков на автомобиль\nУстановка спойлера на автомобиль", "Далее", "Закрыть" );
				}	
			}
		}	
	}
	    ...
	    case d_repair: {
			...
			switch(listitem) {
			    case 0: {
			    	...
			    	showPlayerDialog(playerid,d_cars + 3,1," ",vb_string,"Далее","Закрыть");
			    }
