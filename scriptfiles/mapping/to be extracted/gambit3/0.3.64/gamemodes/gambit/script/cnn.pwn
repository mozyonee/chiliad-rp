...
CMD:n( playerid, params[] ) {
	...
	if(
		...
		|| ( IsPlayerInRangeOfPoint( playerid, 7.0, 1652.4156, -1628.4600, 698.9738 ) ) ) {
		format( vb_string, sizeof vb_string,"[ CNN Эфир ] %s: %s", Player[playerid][uName], params[0]);
		foreach(new i : Player) {
			if(IsPlayerInAnyVehicle(i)) SendClientMessage(i,0x42E73AAA,vb_string);
			else {
				if(Player[i][uSettings][4] == 1) SendClientMessage(i,0x42E73AAA,vb_string);
			}
		}
	}
...
CMD:newspanel( playerid, params[] ) {
	...
	showPlayerDialog(playerid,d_cnn + 2,2," ",vb_string,"Дальше","Закрыть");
	return true;
}
...
hook OnDialogResponse( playerid, dialogid, response, listitem, inputtext[] ) {
	...
	switch( dialogid ) {
		...
		case d_cnn + 1: {
			...
			if(countAdvert == false) return SendClientMessage(playerid,C_GRAY,""gbError"Простите, но в CNN слишком много заявок, приходите позднее!");
		...
		case d_cnn + 2: {
			...
			switch(listitem) {
				case 0: {
					...
					showPlayerDialog( playerid, d_cnn + 3, 2, " ", vb_string, "Дальше", "Закрыть" );
				}
				case 1: {
				...
				case 2: {
					...
					showPlayerDialog(playerid,d_cnn + 7,1," ","{ffffff}Введите ID человека которого хотите пригласить в эфир:","Дальше","Закрыть");
				}
				case 3: {
		...
		case d_cnn + 6: {
			...
			if(sscanf(inputtext, "s[100]",text)) {
				format(gb_string,sizeof(gb_string),""cBLUE"Текст:\n\n{ffffff}%s\n\n"cGRAY"Прислал: "cBLUE"%s"cGRAY" ( "cBLUE"%d "cGRAY")",AD::[GetPVarInt(playerid,"AdvertID")][adText],AD::[GetPVarInt(playerid,"AdvertID")][adName],AD::[GetPVarInt(playerid,"AdvertID")][adPhone]);
				return showPlayerDialog(playerid,d_cnn + 6,1," ",gb_string,"Дальше","Закрыть");
			}
		...
		case d_cnn + 7: {
			if(!response) return 1;
			if(!strlen(inputtext)) return showPlayerDialog(playerid,d_cnn + 7,1," ","{ffffff}Введите ID человека которого хотите пригласить в эфир:","Дальше","Закрыть");
			if(!isNumeric(inputtext)) return showPlayerDialog(playerid,d_cnn + 7,1," ","{ffffff}Введите ID человека которого хотите пригласить в эфир:","Дальше","Закрыть");
			if(strval(inputtext) == playerid) return showPlayerDialog(playerid,d_cnn + 7,1," ","{ffffff}Введите ID человека которого хотите пригласить в эфир:","Дальше","Закрыть");
		...
		case d_cnn + 8: {
			if(response) {
				...
				SendClientMessage(playerid,C_GRAY,""gbInfo"Чтобы писать в эфир, вы должны быть в фургончике CNN. Используйте "cBLUE"/n");
			}
