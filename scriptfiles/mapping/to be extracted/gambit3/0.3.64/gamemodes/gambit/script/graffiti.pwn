...
stock ShowSprayMenu( playerid ) {
	...
	showPlayerDialog(playerid, d_spraytag, DIALOG_STYLE_LIST, "> Граффити", vb_string, "Далее", "Отмена");
	return true;
}
...
publics: AddSprayTag(i, playerid) {
	SprayInfo[i][grID] = cache_insert_id();
	format( gb_string, sizeof gb_string, "[A] Поступило граффити на модерацию от игрока %s[%d]. MysqlID: %d", Player[playerid][uName], playerid, SprayInfo[i][grID]);
...
hook OnDialogResponse( playerid, dialogid, response, listitem, inputtext[] ) {
	...
	switch( dialogid ) {
	    ...
	    case d_spraytag + 1: {
	        if(!response) return true;
	        if( !strlen(inputtext) || strlen(inputtext) > 128 ) return showPlayerDialog(playerid, d_spraytag + 1, DIALOG_STYLE_INPUT, " ", "{FFFFFF}Введите текст граффити. Обращаем Ваше внимание\nчто некоторые шрифты не поддерживают русский язык.", "Далее", "Отмена");
	    ...
	    case d_spraytag + 6: {// Admin
		    ...
		    for(new i; i < MAX_SPRAY; i++) {
			    ...
			    if( SprayInfo[i][grVarificated] == 0 && strlen(SprayInfo[i][grAuthorName]) > 2) {
			        ...
			        showPlayerDialog(playerid, d_spraytag + 7, DIALOG_STYLE_LIST, "> Граффити", "[1] > Телепортироваться\n[2] > Посмотреть информацию\n[3] > Одобрить\n[4] > Удалить", "Далее", "Отмена");
			        break;
			    }
			}
	    }
	    case d_spraytag + 7: {
		    if(!response) return true;
		    new id = GetPVarInt(playerid, "AdminSprayIndex"), ids = GetPVarInt(playerid, "AdminSprayID");
		    switch(listitem) {
		        case 0: {
		        ...
		        case 1: {
		            format( vb_string, sizeof vb_string, "Текс: %s\nАвтор: %s\nШрифт: %s\nРазмер шрифта: %d\nID: %d", 
						SprayInfo[id][grText], SprayInfo[id][grAuthorName], SprayInfo[id][grFont], SprayInfo[id][grFontSize], SprayInfo[id][grID]);
                    showPlayerDialog( playerid, -1,0, "Информация", vb_string, "Выход", "" );
		        }
		        case 2: {
                    ...
                    SendClientMessage( playerid, -1, "Граффити одобрено!" );
