	...
	if( Player[playerid][uHouse] == HouseInfo[house][hID] 
		...
		if( !HouseInfo[house][hInterior] ) {
			showPlayerDialog( playerid, d_house_panel, DIALOG_STYLE_LIST, "Управление квартирой", 
				...
				\n{ffffff}5. "cGRAY"Жильцы в квартире", "Дальше", "Закрыть" );
		}
		if( HouseInfo[house][hEnterWorld] ) {
			showPlayerDialog( playerid, d_house_panel, DIALOG_STYLE_LIST, "Управление квартирой", 
				...
				\n{ffffff}6. "cGRAY"Настройка интерьера [ALPHA TEST]", "Дальше", "Закрыть" );
		}
...
publics: checkHouseEvict( playerid, h ) {
	clear_massive();
			...
			switch( listitem ) {
				...
				case 2: { 
					showPlayerDialog(playerid, d_house_panel + 1, DIALOG_STYLE_LIST, "Управление сейфом", 
						...
						\n{ffffff}3. "cGRAY"Взять из сейфа", "Дальше", "Назад");
				}
				case 3: {
			...
			switch( listitem ) {
				...
				case 1: {
					showPlayerDialog( playerid, d_house_panel + 3, DIALOG_STYLE_INPUT, 
						"Положить в сейф",
						"{ffffff}Введите кол-во денег которое хотите положить в сейф:", "Дальше", "Закрыть" );
				}
				case 2: {
	...
	for( new h; h < sizeof HouseInfo; h++ ) {
		if( HouseInfo[h][hID] == Player[playerid][uHouse] ) {
			if( HouseInfo[h][hOwned] == Player[playerid][uID] ) {
				...
				SendMes( playerid, C_GRAY, "{1081C7}>> {FAEBD7}Вы предложили игроку %s поселиться у Вас в доме", Player[params[0]][uName] );
				SendMes( params[0], C_GRAY, "{1081C7}>> {FAEBD7}Игрок %s хочет поселить Вас в его дом", Player[playerid][uName] );
				SendMes( params[0], C_GRAY, "{1081C7}>> {FAEBD7}Нажмите {33AA33}Y{FAEBD7}, чтобы согласиться или {FF6347}N{FAEBD7}, чтобы отказаться" );
				return true;
			}
		}
	}
