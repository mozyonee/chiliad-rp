	...
	for( new h; h < MAX_HOUSE; h++ ) {
		...
		if( gettime() > HouseInfo[h][hHouseTaxTime] + ( HouseInfo[h][hHouseTax] * 86400 ) && HouseInfo[h][hOwned] ) {
			...
			foreach( new i: Player) {
				if( Player[i][uHouse] == HouseInfo[h][hID] 
					...
					switch( Player[i][uPremium] ) {
						case 1: price = floatround( HouseInfo[h][hPrice] * 0.7 );
						case 2: price = floatround( HouseInfo[h][hPrice] * 0.8 );
						case 3: price = floatround( HouseInfo[h][hPrice] * 0.9 );
						default: price = floatround( HouseInfo[h][hPrice] * 0.6 );
					}	
					SendMes( i, C_GRAY, ""gbInfo"Ваш дом был продан за неуплату государству, сумма $%d перечислена на ваш счёт", 
						price );
					Player[i][uBank] += price;// 70 процентов вернули
					Player[i][uHouse] = 0, userUpdate( i, "uHouse", Player[i][uHouse] );
			...
			if( !gcash ) {
				...
				mysql_tquery( mysql, query, "updateUserBank", "dd", HouseInfo[h][hOwned], HouseInfo[h][hPrice] );
			}
...
publics: updateUserBank( id, price ) {
	new rows, fields;
    cache_get_data( rows, fields );
	if( rows ) {
		new premium, bank;
		...
		format( query, sizeof query, "UPDATE `gb_users` SET `uBank` = '%d' WHERE `uID` = '%d'", bank + price, id );
		mysql_query( mysql, query );
		printf( "[HOUSE] Дом слетел! Игрок %d, дало денег %d", id, bank + price );
	}
	return true;
}
