...
hook OnDialogResponse( playerid, dialogid, response, listitem, inputtext[] ) {
	...
	switch( dialogid ) {
		...
		case d_bank + 5: {
		    ...
		    if(!strlen(inputtext)) return showPlayerDialog(playerid, d_bank + 5, 1, "Перевод денег", "{ffffff}Введите ID человека которому хотите перевести деньги:", "Дальше", "Закрыть");
		    ...
		    if(!isNumeric(inputtext)) return showPlayerDialog(playerid, d_bank + 5, 1, "Перевод денег", "{ffffff}Введите ID человека которому хотите перевести деньги:", "Дальше", "Закрыть");
		...
		case d_bank + 6: {
			...
			logs( 4, "перевёл деньги", Player[playerid][uID], Player[id][uID], strval( inputtext ) );
			format( gb_string, sizeof gb_string, 
				"[A] %s[%d][%s] перевёл деньги через банк %s[%d][%s] в размере %d", 
				Player[playerid][uName], playerid, Player[playerid][uIP], Player[id][uName], 
				id, Player[id][uIP], strval( inputtext ) );
...
hook OnPlayerKeyStateChange( playerid, newkeys, oldkeys ) {
	if( PRESSED( KEY_WALK ) ) {
		if( getData( playerid, "ATM:Use" ) ) {
			...
			else if( Player[playerid][uPremium] ) {
				showPlayerDialog( playerid, d_bank + 1, DIALOG_STYLE_LIST, "Управление личным счетом", 
					"1. "cGRAY"Проверить баланс на счету{ffffff}\n2. "cGRAY"Снять деньги со счета\
					\n{ffffff}3. "cGRAY"Положить деньги на счет\n{ffffff}4. "cGRAY"Перевести деньги другому человеку\
					\n{ffffff}5. "cGRAY"Управление чековой книжкой",
		...
		if( IsPlayerInRangeOfPoint( playerid, 1.0, 1463.8583, -1006.0172, 725.8800 ) 
			|| IsPlayerInRangeOfPoint( playerid, 1.0, 1459.5745, -1006.0161, 725.8800 )
			|| IsPlayerInRangeOfPoint( playerid, 1.0, 1455.4482, -1006.0210, 725.8800 ) 
			|| IsPlayerInRangeOfPoint( playerid, 1.0, 1451.4005, -1006.0151, 725.8800 ) ) {
            
			showPlayerDialog( playerid, d_bank + 1, DIALOG_STYLE_LIST, "Управление личным счетом", 
				"1. "cGRAY"Проверить баланс на счету{ffffff}\
				\n2. "cGRAY"Снять деньги со счета\n{ffffff}3. "cGRAY"Положить деньги на счет\
				\n{ffffff}4. "cGRAY"Перевести деньги другому человеку\
				\n{ffffff}5. "cGRAY"Управление чековой книжкой", "Дальше", "Закрыть");
			return true;
	    }
	}
	return true;
...
hook OnGameModeInit( ) {
	CreateBankomat(1740.29,-1771.58,13.37,0.00,0.00,0.00);
	CreateBankomat(1740.29,-1773.58,13.37,0.00,0.00,0.00);
	CreateBankomat(1772.44,-1773.58,13.37,0.00,0.00,180.00);
	CreateBankomat(1772.44,-1771.58,13.37,0.00,0.00,180.00);
