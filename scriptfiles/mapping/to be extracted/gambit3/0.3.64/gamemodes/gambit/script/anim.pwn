...
hook OnDialogResponse( playerid, dialogid, response, listitem, inputtext[] ) {
	...
	switch( dialogid ) {
		...
		case d_anim + 1: {
			...
			if( listitem == 10 ) {
				...
				for( new id; id < 10; id++ ) {
					...
					if( Player[playerid][uAnimList][id] ) {
						if( !Player[playerid][uAnimListParam][id] ) format( gb_string, sizeof gb_string, ""cGRAY"%d. "cBLUE"%s\n", slot + 1, anim_id[ Player[playerid][uAnimList][id] ][anName] ), strcat( vb_string, gb_string );
						else format( gb_string, sizeof gb_string, ""cGRAY"%d. "cBLUE"%s %d\n", slot + 1, anim_id[ Player[playerid][uAnimList][id] ][anName], Player[playerid][uAnimListParam][id] ), strcat( vb_string, gb_string );
				...
				ShowPlayerDialog( playerid, d_anim + 2, DIALOG_STYLE_LIST, "Избранные анимации | Удаление", vb_string, "Выбор", "Выход" );
				return true;
			}
			...
			if( i == -1 ) {
				if( listitem == GetPVarInt( playerid, "anim_list" ) ) {
					...
					for( new id; id < 10; id++ ) {
						favorite_anim[playerid][slot][0] = -1;
						if( Player[playerid][uAnimList][id] ) {
							if( !Player[playerid][uAnimListParam][id] ) {
								format( str, sizeof str, ""cGRAY"%d. "cBLUE"%s\n", 
									slot + 1, anim_id[ Player[playerid][uAnimList][id] ][anName] ), 
								strcat( vb_string, str );
							}	
							else {
								format( str, sizeof str, ""cGRAY"%d. "cBLUE"%s %d\n", 
									slot + 1, 
									anim_id[ Player[playerid][uAnimList][id] ][anName], 
									Player[playerid][uAnimListParam][id] ), 
								strcat( vb_string, str );
							}
							favorite_anim[playerid][slot][0] = id, 
							favorite_anim[playerid][slot][1] = Player[playerid][uAnimListParam][id];
							slot++;
						}
					}
					//printf( "%s", vb_string );
			...
			format( vb_string, sizeof vb_string, "cmd_%s", str );
			format( str, sizeof str, "%d", param );
			CallLocalFunction( vb_string, "is", playerid, str );		
		}
		case d_anim + 2: {
		...
		case d_anim + 3: {
			if(response) {
				...
				dialog_anticheat[playerid] = d_anim + 3;
				return true;
			}	
			new idx = 22 * getData( playerid, "AnimList:Page" );
			if( getData( playerid, "AnimList:Page" ) == 4 ) {
				for( new i = idx; i < idx + 3; i++ ) strcat( vb_string, anim_list[i] );
				DeletePVar( playerid, "AnimList:Page" );
				showPlayerDialog(playerid, d_anim + 3, DIALOG_STYLE_MSGBOX, "Список анимаций", vb_string, "Закрыть", "");
			}
			else {
				for( new i = idx; i < idx + 22; i++ ) strcat( vb_string, anim_list[i] );
...
CMD:favanim( playerid, params[] ) {
	new str[64], slot;
	format( vb_string, sizeof vb_string, ""cGRAY"" );
	for( new i; i < 10; i++ ) {
		favorite_anim[playerid][i][0] = -1;
		if( Player[playerid][uAnimList][i] ) {
			if( !Player[playerid][uAnimListParam][i] ) format( str, sizeof str, ""cGRAY"%d. "cBLUE"%s\n", slot + 1, anim_id[ Player[playerid][uAnimList][i] ][anName] ), strcat( vb_string, str );
			else format( str, sizeof str, ""cGRAY"%d. "cBLUE"%s %d\n", slot + 1, anim_id[ Player[playerid][uAnimList][i] ][anName], Player[playerid][uAnimListParam][i] ), strcat( vb_string, str );
	...
	strcat( vb_string, ""cGRAY"Добавить анимацию\n"cGRAY"Удалить анимацию" );
	showPlayerDialog( playerid, d_anim + 1, DIALOG_STYLE_LIST, "Избранные анимации", vb_string, "Выбор", "Выход" );
	return true;
}
...
stock saveFavoriteAnimation( playerid ) {
	clear_massive();
	format( vb_string, sizeof vb_string, "UPDATE `gb_users` SET \
...
CMD:anim(playerid, params[]) {
    ...
    for( new i; i < 22; i++ ) strcat( vb_string, anim_list[i] );
    return showPlayerDialog( playerid, d_anim + 3, DIALOG_STYLE_MSGBOX, "Список анимаций", vb_string, "Закрыть", ">>>>>>");   
}
