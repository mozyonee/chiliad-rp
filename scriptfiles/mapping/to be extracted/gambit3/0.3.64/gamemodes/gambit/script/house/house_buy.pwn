...
hook OnPlayerKeyStateChange( playerid, newkeys, oldkeys ) {
	switch( newkeys ) {
		case KEY_WALK: {
			if( IsPlayerInRangeOfPoint( playerid, 1.0, 1072.4669,-342.6463,797.7004 ) ) {
			...
			else if(IsPlayerInRangeOfPoint(playerid,1.0,1065.0851,-339.9360,797.7010)) {
		...
		case KEY_YES: {
			...
			if( getData( playerid, "HBuy:Accept" ) ) {
				...
				SendMes( playerid, -1, ""gbAccess"Вы успешно купили дом у игрока %s", Player[sellid][uName] );
				SendMes( sellid, -1, ""gbAccess"Игрок, %s купил у вас дом за $%d", Player[playerid][uName], price );
		...
		case KEY_NO: {
			if( getData( playerid, "HBuy:Accept" ) ) {
				...
				SendMes( playerid, -1, ""gbInfo"Вы отказались от покупки дома у игрока %s", Player[sellid][uName] );
				SendMes( sellid, -1, ""gbInfo"Игрок, %s отказался купить у вас дом за $%d", Player[playerid][uName], getData( playerid, "HBuy:Price" ) );
				delData( playerid, "HBuy:Accept" ), delData( playerid, "HBuy:SellID" ),
				delData( playerid, "HBuy:Price" );
			}
		}
	...
	switch( dialogid ) {
			...
			switch( listitem ) {
				...
				case 1: { // Каталог бизнесов
					showPlayerDialog( playerid, d_bizz_menu, 2, "Центр купли-продажи",
						...
						\n2. "cGRAY"Продать собственный бизнес", "Дальше", "Закрыть" );
				}
			}
		}
		case d_buy_menu + 1: {
		...
		case d_buy_menu + 4: {
			...
			if( response ) {
				...
				if( Player[playerid][uHouse] ) return SendMes( playerid, C_GRAY, ""gbError"У вас уже есть дом!" );
				if( HouseInfo[h][hPrice] > GetPlayerMoney( playerid ) ) return SendMes( playerid, C_GRAY, ""gbError"У вас недостаточно средств для покупки этого дома." );
				...
				SendMes( playerid, C_GRAY, ""gbAccess"Вы купили собственный дом, ваш дом автоматически оплачен на "cBLUE"2{ffffff} дня.");
		...
		case d_buy_menu + 6: {
			...
			Player[playerid][uMoney] += getData( playerid, "HBuy:Cash" );
			...
			SendMes( playerid, C_GRAY, ""gbInfo"Вы успешно продали дом за $%d", getData( playerid, "HBuy:Cash" ) );
			Player[playerid][uHouse] = 0;
			format( query, sizeof query, "UPDATE `gb_users` SET `uHouse` = '0' WHERE `uHouse` = '%d'",
				HouseInfo[h][hID] );
			mysql_query( mysql, query );
			
			format( HouseInfo[h][hOwner], 32, "None" ); houseUpdateStr( HouseInfo[h][hID], "hOwner", HouseInfo[h][hOwner] );
			HouseInfo[h][hOwned] = 0, houseUpdate( HouseInfo[h][hID], "hOwned", HouseInfo[h][hOwned] );
		...
		case d_buy_menu + 10: {
			...
			if( Player[ inputtext[0] ][uHouse] ) {
				SendMes( playerid, -1, ""gbError"У игрока уже есть дом или он снимает комнату" );
				status = true;
			}
			if( status ) {
				showPlayerDialog( playerid, d_buy_menu + 10, DIALOG_STYLE_INPUT, "Продать дом игроку",
