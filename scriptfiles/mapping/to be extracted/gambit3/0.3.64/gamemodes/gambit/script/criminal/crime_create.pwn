...
hook OnDialogResponse( playerid, dialogid, response, listitem, inputtext[] ) {
	...
	switch( dialogid ) {
		...
		case d_crime_cr + 8: {
			if( !response ) return showCrimeEdit( playerid, cr_id );
			showCrimeEdit( playerid, cr_id );
			SendMes( playerid, -1, ""gbInfo"Данная функция тестируется! Поэтому пока недоступна" );
			/*
			format( query, sizeof query, "DELETE FROM `gb_crime` WHERE `crID` = '%d'", CrimeInfo[cr_id][crID] );
			mysql_query( mysql, query );
			format( query, sizeof query, "DELETE FROM `gb_crime_rank` WHERE `raCrime` = '%d'", CrimeInfo[cr_id][crID] );
			mysql_query( mysql, query );
			format( query, sizeof query, "UPDATE `gb_users` SET `uCrimeM` = '0', `uCrimeL` = '0', \
			...
			foreach( new i: Player) {
				if( Player[i][uCrimeM] == CrimeInfo[cr_id][crID] || Player[i][uCrimeL] == CrimeInfo[cr_id][crID] ) {
					...
					for( new s; s < 10; s++ ) Player[i][uCrimeSet][s] = 0;
				}
			}			
			mysql_query( mysql, query );
			for( new c; c < MAX_VEHICLES; c++ ) {
				if( V::[c][vCrime] == CrimeInfo[cr_id][crID] ) {
					DestroyVehicle( c );
					V::[c][vCrime] = 0;
					for( new i; i < 10; i++ ) CrimeCar[c][i][cr_vID] = 0;
				}
			}
			for( new i; i < 20; i++ ) CrimeRank[cr_id][i][raID] = 0;
			CrimeInfo[cr_id][crID] = 0;
			SendMes( playerid, -1, ""gbAccess"Вы успешно удалил фракцию!" );*/
		}
	}
	return true;
}
...
publics: insertServerCrime( playerid, cr ) {
