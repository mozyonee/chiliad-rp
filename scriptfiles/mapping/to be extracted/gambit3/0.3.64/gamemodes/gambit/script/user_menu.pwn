...
stock showMenu( playerid ) {
	...
	switch( Player[playerid][uCriminal] ) {
		case 0..500: format( query,sizeof query,"A");
		case 501..1000: format( query,sizeof query,"B");
		case 1001..1500: format( query,sizeof query,"C");
		case 1501..2000: format( query,sizeof query,"D");
		case 2001..3000: format( query,sizeof query,"S");
		default: format( query,sizeof query,"F");
	}
	format( gb_string, sizeof gb_string, "~n~%s", query ), strcat( vb_string, gb_string );
