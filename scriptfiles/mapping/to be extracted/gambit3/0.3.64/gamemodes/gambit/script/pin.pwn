...
hook OnPlayerClickTextDraw(playerid, Text:clickedid) {
	if(_:clickedid == INVALID_TEXT_DRAW) {
	    if( getData(playerid, "Login:aloginTD") ) {
			...
			SendClientMessage(playerid, C_LIGHTRED, "Вы были кикнуты за отказ ввода пароля!");
			KickEx(playerid);
		}
	}
	return true;
...
hook OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid) {
	...
	if(playertextid == AloginTextDraw[playerid][12]) {
	    if( strval(PlayerKey[playerid]) == Player[playerid][uPinCode] ) {
	        CancelSelectTextDraw(playerid);
			for(new i; i < 17; i++) PlayerTextDrawHide( playerid, AloginTextDraw[playerid][i] );
	        SendClientMessage(playerid, C_GRAY, "Вы успешно прошли двухфакторную аутентификацию!");
	        PlayerKey[playerid] = "";
	        ...
	        SendClientMessage(playerid, C_LIGHTRED, "Вы были кикнуты за неправильный ввод пароля!");
	        KickEx(playerid);
	    }
		//Function
	}
