...
publics: insertRank( playerid, i, id ) {
...
publics: crimeMemberList( playerid, id ) {
	clear_massive();
...
publics: selectCrimeMember( playerid, id ) {
	clear_massive();
	new rows, f, uid; 
	cache_get_data( rows, f );
	if( rows ) {
...
hook OnDialogResponse( playerid, dialogid, response, listitem, inputtext[] ) {
	...
	switch( dialogid ) {
		case d_crime - 1: {
			...
			GivePlayerCriminal(playerid, 200);
		}
		case d_crime: { // mpanel
