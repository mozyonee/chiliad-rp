...
CMD:set_house_interior( playerid, params[] ) {
	if( Player[playerid][uAdmin] < 7 ) return true;
	...
	for( new h; h < MAX_HOUSE; h++ ) {
		if( IsPlayerInRangeOfPoint( playerid, 1.5, 
			...
			loadHouseInterior( h );
			SendMes( playerid, -1, "Вы установили дому %d, интерьер %s", HouseInfo[h][hID], house_int[ params[0] - 1 ][ht_name] );
			break;
		}
	}
	return true;
...
stock loadHouseInterior( id ) {
	...
	switch( int ) {
		...
		case 11: {
			...
			CreateDynamicObject(14394, -1620.65, 496.49, 1020.43,   0.00, 0.00, 90.00, world, 5 );// ‹естница
			CreateDynamicObject(14394, -1615.85, 501.31, 1022.13,   0.00, 0.00, 0.00, world, 5 );// ‹естница
